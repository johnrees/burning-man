...
var x = radius + 50;
var y = radius / 2 + 50;
var ROADS = ["Leon", "Kinoshita", "Johnny 5", "Iron Giant", "HAL", "Gort", "Fribo", "Elektro", "Dewey", "Cylon", "Bender", "Algorithm", "Esplanade"];
// 2-10
// function Road({ index }) {
...
function Road(_a) {
    ...
    var a1 = Math.PI / 6 * index - Math.PI / 2;
    var x1 = x + Math.cos(a1) * start;
    var y1 = y + Math.sin(a1) * start;
    var x2 = x + Math.cos(a1) * end;
    var y2 = y + Math.sin(a1) * end;
    return React.createElement("line", { x1: x1, x2: x2, y1: y1, y2: y2 });
}
...
function Line(_a) {
    var index = _a.index;
    var angle = Math.PI / 6 * index - Math.PI / 2;
    var x2 = x + Math.cos(angle) * radius;
    var y2 = y + Math.sin(angle) * radius;
    return React.createElement("line", { x1: x, y1: y, x2: x2, y2: y2, stroke: "black" });
}
// 2PI = circle
// PI = half circle
// PI/6 = 1 clock symbol
...
function Arc(_a) {
    var r = _a.r,
        road = _a.road;
    var a1 = Math.PI / 6 * 2 - Math.PI / 2;
    var x1 = x + Math.cos(a1) * r;
    var y1 = y + Math.sin(a1) * r;
    var a2 = Math.PI / 6 * 10 - Math.PI / 2;
    ...
    return React.createElement(
        ...
        React.createElement("path", { d: "M " + x1 + " " + y1 + " A " + r + " " + r + " 0 1 1 " + x2 + " " + y2 })
    );
}
function RoadLabel(_a) {
    var i = _a.i;
    var a = Math.PI / 6 * i - Math.PI / 2;
    var dist = radius + 20;
    var x1 = x + Math.cos(a) * dist - 12;
    var y1 = y + Math.sin(a) * dist;
    var _b = i.toString().split(".").map(function (x) {
        return Number(x);
    }),
        hour = _b[0],
        minute = _b[1];
    return React.createElement(
        "text",
        { x: x1, y: y1, anchor: "middle" },
        hour,
        ":",
        (60 * minute || "00").toString().slice(0, 2)
    );
}
function BlackRockCity() {
    console.log("RENDER BRC");
    var roads = [];
    for (var i = 2.25; i < 10; i += 0.5) {
        roads.push({ i: i, start: 375, end: 500 });
    }
    for (var i = 2; i <= 10; i += 0.5) {
        roads.push({ i: i, start: 200, end: 500 });
    }
    console.log(roads);
    var roadLabels = [];
    for (var i = 2; i <= 10; i += 0.25) {
        roadLabels.push(i);
    }
    return React.createElement(
        "svg",
        { width: radius * 2 + 100, height: radius * 2 + 100 },
        React.createElement("circle", { cx: x, cy: y, r: 12, fill: "none", stroke: "#666" }),
        React.createElement(
            "g",
            { className: "roads" },
            ROADS.map(function (road, index) {
                return React.createElement(Arc, { key: road, road: road, r: radius - index * 25 });
            }),
            roads.map(function (road) {
                return React.createElement(Road, { index: road.i, start: road.start, end: road.end });
            }),
            roadLabels.map(function (i) {
                return React.createElement(RoadLabel, { key: "roadlabel" + i, i: i });
            }),
            React.createElement("circle", { cx: x, cy: y + 225, r: 56, fill: "#333333" }),
            React.createElement(Arc, { road: ROADS[11], r: radius - 11 * 25 })
        )
    );
}
react_dom_1.render(React.createElement(BlackRockCity, null), document.getElementById("root"));
...
//# sourceMappingURL=/burning-man.f53f4a7d.map